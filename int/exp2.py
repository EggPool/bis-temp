...
def balance_from_cursor(cursor, address):
    credit = Decimal("0")
    debit = Decimal("0")
    for entry in execute_param(cursor, "SELECT amount,reward FROM transactions2 WHERE recipient = ? ",(address, )):
        try:
    ...
    for entry in execute_param(cursor, "SELECT amount,fee FROM transactions2 WHERE address = ? ",(address, )):
        try:
            # result = cursor.fetchall()
...
def balance_from_intcursor(cursor, address):
    q1 = execute_param(cursor, "SELECT sum(iamount+ireward) FROM transactions2 WHERE recipient = ? ",(address, ))
    r1 = q1.fetchone()
    credit = r1[0] if r1[0] else 0
    if VERBOSE:
        print ("Credit2", credit)
    q2 = execute_param(cursor, "SELECT sum(iamount+ifee) FROM transactions2 WHERE address = ? ",(address, ))
    r2 = q2.fetchone()
    ...
    return "{:.8f}".format(Decimal(credit-debit)/essentials.DECIMAL_1E8)
